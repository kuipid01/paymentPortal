...
const Payment = () => {
  ...
  const confirmPayment = async () => {
    try {
      ...
      if (userDoc.exists()) {
        // Check if resultObject.data exists before destructuring
        const amount = resultObject.data ? resultObject.data.amount : 0;

        const totalBalance = userDoc.data().totalBalance;

        //Update totalBalance
        await updateDoc(userRef, {
          totalBalance: totalBalance - parseInt(amount, 10),
        });
